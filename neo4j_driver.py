...
class Neo4jInteractive:
        ...
        while True:
            ...
            try:    
                if query=='1':
                    ...
                    results = self.create_Analyst(queryArr[0], queryArr[1], queryArr[2], queryArr[3])
                    print(results)
    ...
    def create_Analyst(self, Name, ID, role, initials):
        ...
        if not all([Name, ID, role, initials]):
            return json().set_data({"error": "Missing required parameters"}).build()
        #initial query to create user
        query="CREATE (u: Analyst {id: $Id, name: $name, role: $role, initials: $initials})"
        with self.driver.session() as session:
            #run query with params in safe way using $
            session.run(query, name=str(Name), Id=int(ID), role=str(role), initials=str(initials))
            #return as variables to format it into json
            result = session.run("MATCH (u: Analyst) RETURN u.id AS id, u.name AS name, u.role AS role, u.initials AS initials")
            leads = [{"Id": record["id"], "Name": record["name"], "role": record["role"], "initials": record["initials"]} for record in result]
    ...
    def print_Lead(self):
        with self.driver.session() as session:
            ...
            result= session.run("MATCH (u:User) RETURN u.id AS id, u.name AS name, u.role AS role")
            users= [{"ID": record["id"], "Name": record["name"], "Role": record["role"]}for record in result]
            for user in users:
                print(user)
    ...
    def join_to_Project(self, Project_Name):
        return 0
